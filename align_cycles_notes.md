...
**Behavioral 端**：从 `.json.log` 中筛选 `msg == "Inst"` 且 predicate 为 True 的记录，按 `(X, Y, ID)` 分组保存每个 tile 的触发时刻列表。支持 `MUL`、`STORE`、`ADD` 三类操作。
...
去除这两项开销后，稳态阶段的 `rtl_corrected` 与 `behavioral_time` 偏差在 sub-cycle 量级（< 2 cycles），验证了 behavioral 仿真器对稳态行为的正确性。
...
| STORE | 4 | 15 | 4 | 0.318 | 1.0（但含义错误） | ✗ 粒度不匹配（见下） |

**Tile 选择**：ADD 操作选中 `behav tile(X=1,Y=1,ID=41) ↔ RTL tile(id=5,row=1,col=1)`，事件数完全吻合。

**MUL 稳态线性拟合参数**（用于输出 `rtl_corrected`）：
- slope = 1.2659，intercept = 181.87，R² = 0.9999
- 含义：RTL MUL 周期 ≈ 1.266 × behavioral_time + 181.87

**GEMV STORE 的说明**：behavioral STORE 只在 predicate=True 时触发（即每积累满 4 次 MAC 后写出一次结果，共 4 次），而 RTL `(st)` tile 每次迭代都有 valid handshake（共 15 次），两者粒度本质不同，无法做 1:1 对应，不应作为对齐锚点使用。
...
**Tile 选择**：ADD 操作选中 `behav tile(X=2,Y=0,ID=181) ↔ RTL tile(id=2,row=0,col=2)`。GEMM 的 ADD 指令被多个 tile 共用（累加、地址计算、循环控制各有独立 tile），联合 tile 选择正确过滤出了累加用的那一对。

**GEMM 所有三个操作的稳态线性拟合参数**（几乎完全一致）：
...
| 字段 | 说明 |
|------|------|
| `behavioral_time` | behavioral 仿真器中该事件的触发时刻（单位：behavioral cycle） |
| `rtl_cycle` | RTL 仿真器中对应事件的触发周期 |
...
可选参数：
- `--ops MUL,ADD`：只对齐指定操作（例如跳过 GEMV 的 STORE）
